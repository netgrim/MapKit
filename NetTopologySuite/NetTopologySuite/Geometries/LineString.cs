...
namespace NetTopologySuite.Geometries
{
    ...
    public class LineString : Geometry, ILineString
    {
        ...
        private GeoAPI.Geometries.OgcGeometryType _geometryType;
		...
		public ICoordinateSequence Vertices
		{
			get
			{
				return _points;
            ...
            : base(factory)
        {
            ...
            //if (points.Count == 1)
            //    throw new GeometryException("Invalid number of points in LineString (found "
            //          + points.Count + " - must be 0 or >= 2)");
            _points = points;
            _geometryType = ToGeometryType(points.Ordinates);
        }

        private GeoAPI.Geometries.OgcGeometryType ToGeometryType(Ordinates ordinates)
        {
            switch (ordinates)
            {
                case Ordinates.XY: return OgcGeometryType.LineString;
                case Ordinates.XYZ: return OgcGeometryType.LineStringZ;
                case Ordinates.XYM: return OgcGeometryType.LineStringM;
                case Ordinates.XYZM: return OgcGeometryType.LineStringZM;
