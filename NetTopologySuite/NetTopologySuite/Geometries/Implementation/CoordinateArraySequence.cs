...
namespace NetTopologySuite.Geometries.Implementation
{
    ...
    {    
        ...
        public CoordinateArraySequence(Coordinate[] coordinates) 
        {
			...
			else
			{
				...
				for (int i = 0; i < length; i++)
				{
					...
					var ordinates = coordinate.GetOrdinates();
        ...
        public CoordinateArraySequence(ICoordinateSequence coordSeq)
        {
			...
			{
				var ordinateArray = coordSeq.GetCoordinate(i).GetOrdinates();
				for (int j = 0; j < dimension; j++)
					_coordinates[i,j] = ordinateArray[j];
			}
        }
		...
		public Coordinate GetCoordinate(int i)
		{
			switch (_ordinate)
			{
                ...
                case SupportedOrdinate.XYM:
					return new CoordinateM(_coordinates[i, 0], _coordinates[i, 1], _coordinates[i, 2]);
                case SupportedOrdinate.XYZ:
					return new CoordinateZ(_coordinates[i, 0], _coordinates[i, 1], _coordinates[i, 2]);
                case SupportedOrdinate.XYZM:
					return new CoordinateZM(_coordinates[i, 0], _coordinates[i, 1], _coordinates[i, 2], _coordinates[i, 3]);
				default:
					throw new InvalidEnumArgumentException("Ordinates must represent 2,3 or 4 dimensions");
			}
		}
