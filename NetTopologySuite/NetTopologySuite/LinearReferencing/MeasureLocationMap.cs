...
namespace NetTopologySuite.LinearReferencing
{
    ...
    public class MeasureLocationMap
    {
        ...
        public LinearLocation GetLocation(double measure, bool resolveLower = true)
        {
            ...
            if (!startMeasure.HasValue || !endMeasure.HasValue)
        ...
        private LinearLocation GetLocationAscending(double measure)
        {
            ...
            while (lo <= hi)
            {
                ...
                if (measure < (startMeasure = medianGeom.GetCoordinateN(0).M.Value))
                    hi = median - 1;
                else if (median > (endMeasure = medianGeom.GetCoordinateN(medianGeom.NumPoints - 1).M.Value))
                    lo = median + 1;
        ...
        private LinearLocation GetLocationAscending(int componentIndex, ILineString line, double measure)
        {
            ...
            var loMeasure = line.GetCoordinateN(lo).M.Value;
            var hiMeasure = line.GetCoordinateN(hi).M.Value;
