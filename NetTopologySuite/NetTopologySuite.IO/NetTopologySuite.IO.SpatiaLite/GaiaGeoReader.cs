...
namespace NetTopologySuite.IO
{
    public class GaiaGeoReader : IBinaryGeometryReader
    {
        ...
        public IGeometry Read(byte[] blob, int index, int count)
        {
            ...
            offset = index + 39;
            var geom = ParseWkbGeometry(blob, ref offset, factory, gaiaImport);
            if (geom != null)
            {
                geom.SRID = srid;
                //geom.Envelope = env;
        ...
        private static IGeometry ParseWkbGeometry(byte[] blob, ref int offset, IGeometryFactory factory, GaiaImport gaiaImport)
        {
			...
			switch (type)
			{
				case GaiaGeoGeometry.GAIA_POINT:
					return ParseWkbPoint(blob, ref offset, factory, ReadXY, gaiaImport);
				case GaiaGeoGeometry.GAIA_LINESTRING:
                    return ParseWkbLineString(blob, ref offset, factory, ReadXY, gaiaImport);
				case GaiaGeoGeometry.GAIA_POLYGON:
					return ParseWkbPolygon(blob, ref offset, factory, ReadXY, gaiaImport);
				case GaiaGeoGeometry.GAIA_MULTIPOINT:
					return ParseWkbMultiPoint(blob, ref offset, factory, ReadXY, gaiaImport);
				case GaiaGeoGeometry.GAIA_MULTILINESTRING:
					return ParseWkbMultiLineString(blob, ref offset, factory, ReadXY, gaiaImport);
				case GaiaGeoGeometry.GAIA_MULTIPOLYGON:
					return ParseWkbMultiPolygon(blob, ref offset, factory, ReadXY, gaiaImport);
				case GaiaGeoGeometry.GAIA_POINTZ:
					return ParseWkbPoint(blob, ref offset, factory, ReadXYZ, gaiaImport);
				case GaiaGeoGeometry.GAIA_LINESTRINGZ:
                    return ParseWkbLineString(blob, ref offset, factory, ReadXYZ, gaiaImport);
				case GaiaGeoGeometry.GAIA_POLYGONZ:
					return ParseWkbPolygon(blob, ref offset, factory, ReadXYZ, gaiaImport);
				case GaiaGeoGeometry.GAIA_MULTIPOINTZ:
					return ParseWkbMultiPoint(blob, ref offset, factory, ReadXYZ, gaiaImport);
				case GaiaGeoGeometry.GAIA_MULTILINESTRINGZ:
					return ParseWkbMultiLineString(blob, ref offset, factory, ReadXYZ, gaiaImport);
				case GaiaGeoGeometry.GAIA_MULTIPOLYGONZ:
					return ParseWkbMultiPolygon(blob, ref offset, factory, ReadXYZ, gaiaImport);
				case GaiaGeoGeometry.GAIA_POINTM:
					return ParseWkbPoint(blob, ref offset, factory, ReadXYM, gaiaImport);
				case GaiaGeoGeometry.GAIA_LINESTRINGM:
                    return ParseWkbLineString(blob, ref offset, factory, ReadXYM, gaiaImport);
				case GaiaGeoGeometry.GAIA_POLYGONM:
					return ParseWkbPolygon(blob, ref offset, factory, ReadXYM, gaiaImport);
				case GaiaGeoGeometry.GAIA_MULTIPOINTM:
					return ParseWkbMultiPoint(blob, ref offset, factory, ReadXYM, gaiaImport);
				case GaiaGeoGeometry.GAIA_MULTILINESTRINGM:
					return ParseWkbMultiLineString(blob, ref offset, factory, ReadXYM, gaiaImport);
				case GaiaGeoGeometry.GAIA_MULTIPOLYGONM:
					return ParseWkbMultiPolygon(blob, ref offset, factory, ReadXYM, gaiaImport);
				case GaiaGeoGeometry.GAIA_POINTZM:
					return ParseWkbPoint(blob, ref offset, factory, ReadXYZM, gaiaImport);
				case GaiaGeoGeometry.GAIA_LINESTRINGZM:
                    return ParseWkbLineString(blob, ref offset, factory, ReadXYZM, gaiaImport);
				case GaiaGeoGeometry.GAIA_POLYGONZM:
					return ParseWkbPolygon(blob, ref offset, factory, ReadXYZM, gaiaImport);
				case GaiaGeoGeometry.GAIA_MULTIPOINTZM:
					return ParseWkbMultiPoint(blob, ref offset, factory, ReadXYZM, gaiaImport);
				case GaiaGeoGeometry.GAIA_MULTILINESTRINGZM:
					return ParseWkbMultiLineString(blob, ref offset, factory, ReadXYZM, gaiaImport);
				case GaiaGeoGeometry.GAIA_MULTIPOLYGONZM:
					return ParseWkbMultiPolygon(blob, ref offset, factory, ReadXYZM, gaiaImport);
				case GaiaGeoGeometry.GAIA_COMPRESSED_LINESTRING:
                    return ParseWkbLineString(blob, ref offset, factory, ReadCompressedXY, gaiaImport);
				case GaiaGeoGeometry.GAIA_COMPRESSED_POLYGON:
					return ParseWkbPolygon(blob, ref offset, factory, ReadCompressedXY, gaiaImport);
				case GaiaGeoGeometry.GAIA_COMPRESSED_LINESTRINGZ:
					return ParseWkbLineString(blob, ref offset, factory, ReadCompressedXYZ, gaiaImport);
				case GaiaGeoGeometry.GAIA_COMPRESSED_POLYGONZ:
					return ParseWkbPolygon(blob, ref offset, factory, ReadCompressedXYZ, gaiaImport);
				case GaiaGeoGeometry.GAIA_COMPRESSED_LINESTRINGM:
					return ParseWkbLineString(blob, ref offset, factory, ReadCompressedXYM, gaiaImport);
				case GaiaGeoGeometry.GAIA_COMPRESSED_POLYGONM:
					return ParseWkbPolygon(blob, ref offset, factory, ReadCompressedXYM, gaiaImport);
				case GaiaGeoGeometry.GAIA_COMPRESSED_LINESTRINGZM:
					return ParseWkbLineString(blob, ref offset, factory, ReadCompressedXYZM, gaiaImport);
				case GaiaGeoGeometry.GAIA_COMPRESSED_POLYGONZM:
					return ParseWkbPolygon(blob, ref offset, factory, ReadCompressedXYZM, gaiaImport);
        ...
        private static IMultiPoint ParseWkbMultiPoint(byte[] blob, ref int offset, IGeometryFactory factory, ReadCoordinatesFunction readCoordinates, GaiaImport gaiaImport)
        {
            ...
            for (var i = 0; i < number; i++)
            {
				...
				var gt = (GaiaGeoGeometry)getInt32(blob, ref offset);
				if (!IsPoint(gt))
                    throw new System.ComponentModel.InvalidEnumArgumentException();
        ...
        private static IMultiLineString ParseWkbMultiLineString(byte[] blob, ref int offset, IGeometryFactory factory, ReadCoordinatesFunction readCoordinates, GaiaImport gaiaImport)
        {
            ...
            for (var i = 0; i < number; i++)
            {
                ...
                var gt = (GaiaGeoGeometry)gaiaImport.GetInt32(blob, ref offset);
                if (!IsLineString(gt))
                    throw new Exception();
		...
		private static bool IsPoint(GaiaGeoGeometry type)
		{
			switch (type)
			{
				case GaiaGeoGeometry.GAIA_POINT:
		...
		private static bool IsPolygon(GaiaGeoGeometry type)
		{
			switch (type)
			{
				case GaiaGeoGeometry.GAIA_POLYGON:
		...
		private static bool IsLineString(GaiaGeoGeometry type)
		{
			switch (type)
			{
				case GaiaGeoGeometry.GAIA_LINESTRING:
        ...
        private static IGeometryCollection ParseWkbGeometryCollection(byte[] blob, ref int offset, IGeometryFactory factory, GaiaImport gaiaImport)
        {
            ...
            for (var i = 0; i < number; i++)
            {
                ...
                geometries[i] = ParseWkbGeometry(blob, ref offset, factory, gaiaImport);
            }
        ...
        private static ICoordinateSequence ReadCompressedXYZ(byte[] buffer, ref int offset, int number, GaiaImport import, ICoordinateSequenceFactory factory, IPrecisionModel precisionModel)
        {
            ...
            var y = startOrdinateValues[1];
            ret.SetOrdinate(0, Ordinate.Y, precisionModel.MakePrecise(y));
			var z = handleZ ? startOrdinateValues[2] : double.NaN;
            ...
            ret.SetOrdinate(i, Ordinate.X, precisionModel.MakePrecise(startOrdinateValues[0]));
            ret.SetOrdinate(i, Ordinate.Y, precisionModel.MakePrecise(startOrdinateValues[1]));
            z = handleZ ? startOrdinateValues[2] : 0;
            ret.SetOrdinate(i, Ordinate.Z, z);
            return ret;
        }
        ...
        private static ICoordinateSequence ReadCompressedXYM(byte[] buffer, ref int offset, int number, GaiaImport import, ICoordinateSequenceFactory factory, IPrecisionModel precisionModel)
        {
            ...
            var y = startOrdinateValues[1];
            ret.SetOrdinate(0, Ordinate.Y, precisionModel.MakePrecise(y));
            var m = handleM ? startOrdinateValues[2] : double.NaN;
            ...
            ret.SetOrdinate(i, Ordinate.X, precisionModel.MakePrecise(startOrdinateValues[0]));
            ret.SetOrdinate(i, Ordinate.Y, precisionModel.MakePrecise(startOrdinateValues[1]));
			m = handleM ? startOrdinateValues[2] : double.NaN;
            ret.SetOrdinate(i, Ordinate.M, m);
            return ret;
        }
        ...
        private static ICoordinateSequence ReadCompressedXYZM(byte[] buffer, ref int offset, int number, GaiaImport import, ICoordinateSequenceFactory factory, IPrecisionModel precisionModel)
        {
			...
			var z = handleZ ? startOrdinateValues[2] : double.NaN;
            ret.SetOrdinate(0, Ordinate.Z, z);
			var m = handleM ? startOrdinateValues[3] : double.NaN;
            ret.SetOrdinate(0, Ordinate.M, m);
            ...
            ret.SetOrdinate(i, Ordinate.X, precisionModel.MakePrecise(startOrdinateValues[0]));
            ret.SetOrdinate(i, Ordinate.Y, precisionModel.MakePrecise(startOrdinateValues[1]));
			z = handleZ ? startOrdinateValues[2] : double.NaN;
            ret.SetOrdinate(i, Ordinate.Z, z);
			m = handleM ? startOrdinateValues[3] : double.NaN;
            ret.SetOrdinate(i, Ordinate.M, m);
            return ret;
        }
